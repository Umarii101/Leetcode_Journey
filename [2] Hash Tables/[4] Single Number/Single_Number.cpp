...
int singleNumber(std::vector <int>& nums){
    ...
    for (int key : nums){
        if(hashset.count(key) > 0){
            ...
            hashset.erase(key);//Since A number is only Repeated Twice, We dont need to keep it in out Hashset Anymore, Hence Remove it.
        }
        else{
...
int main(){
    ...
    std::cout<<"The Single Number in the Input array is : "<<Result<<"\n";
    return 0;
}
