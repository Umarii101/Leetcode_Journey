...
int removeElement(vector<int>&nums,int val){
    ...
    while(i<last){
        if (nums[i] == val) {
            while(nums[i]==val|| i<last){ //This while loop is used to handle the case for when the next index also contains the same val we want to remove, so we remove the value and do the shift until val is not repeated or the loop ends.
...
int main(){
    ...
    int val=2;
    int length;
    //length = removeElement(nums,val); Commenting this out because it caused "segmentation fault core dumped"
